# -*- coding: utf-8 -*-
"""
Created on Thu Oct 27 11:54:48 2022
...
wd = pathlib.PureWindowsPath(r'C:\Users\s2132627\OneDrive - University of Edinburgh\The University of Edinburgh\PhD\Step 1 - Variograms\Freiberg Gneiss\aperture').as_posix() #flips the backslashes to plug in open()
...
wdsave = pathlib.PureWindowsPath(wd + r'\upscale').as_posix() #flips the backslashes to plug in open()
bs="//"; wdsave=wdsave+bs  
...
def grid_fit(dataframe):
    ...
    dist_df = dataframe[['x', 'y']].copy(); ar = dist_df.to_numpy(); dist_df['xdist_to_closest_pt'] = 0;                            # Create a copy of the xy values of all points. Memory allocation. Initialise needed list to make the tree
...
# =============================================================================
    ...
    # =============================================================================
                    ...
                    if box.empty == False:
                        ...
                        boxvar = box[vcol].var()
                        exec(f"upscale{scale} = upscale{scale}.append( pd.DataFrame( [[bcx, bcy, (C[0]*bcx + C[1]*bcy + C[2]), boxavg, boxvar, boxstd ]], columns= ['x','y', vcol, 'avg', 'var', 'std']) )")   # Append line (point) of the centre of the box to the DataFrame for that scale.
    ...
    if save == True:
        ...
        ################################################################################################
        exec(f'mainpd.to_csv("{wdsave}{inputfilename}_upscaled_{vcol}_Fracs{extension_csv}")')  # 
