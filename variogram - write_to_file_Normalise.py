...
inputfilename = pathlib.PureWindowsPath(r'GW1Q4').as_posix()

# =============================================================================
# wd = pathlib.PureWindowsPath(r'C:').as_posix() #flips the backslashes to plug in open()
...
wdsave = pathlib.PureWindowsPath(r'C:\Users\s2132627\OneDrive - University of Edinburgh\The University of Edinburgh\PhD\Step 1 - Variograms\Greywacke scans\GW1_Q4').as_posix() #flips the backslashes to plug in open()
bs="//"; wdsave=wdsave+bs  
...
###########################################################################################################################################################'''
df= pd.read_csv(wd + inputfilename + extension_csv, index_col = 0)                     # read a .csv file in as a DataFrame. The input file needs to be in a table format, i.e. columns of 'x', 'y', 'z' with #rows = #points. As is, it only reads columns 0,1,2 and ignores the rest.
# df= pd.read_csv(wd + inputfilename + extension_txt, usecols = [0,1,2], names = ["x","y","z"])

xtotal = len(df)     #checks how many unique values are in the first column of the array, i.e. how many rows there are in the dataset.
ytotal = len(df.columns)     #as as above for columns.
total = xtotal # number of points



df[f'N{vcol}'], tvz, tnsz = nscore(df, vcol) #  This creates a Z-score of the line(from scipy.stats)    Or in numpy: line_Z= pd.DataFrame(np.abs(stats.zscore(line))).  https://towardsdatascience.com/ways-to-detect-and-remove-the-outliers-404d16608dba

stats=df.describe().transpose(); xmin = stats['min']['x']; xmax = stats['max']['x']; ymin = stats['min']['y']; ymax = stats['max']['y'];
vmin = stats['min'][vcol]; vmax = stats['max'][vcol]; nvmin = stats['min'][f'N{vcol}']; nvmax = stats['max'][f'N{vcol}']
...
azi_mat = [67.5]           # directions in azimuth to consider
...
plt.hist(df[f'{vcol}'], facecolor='red',bins=np.linspace(vmin,vmax,1000), histtype="stepfilled",alpha=0.2,density=True,cumulative=False,edgecolor='black',label='Original')
# plt.xlim([0.05,0.25
# plt.ylim([0,5.0])
plt.xlabel(f'{vcol}'); plt.ylabel('Frequency'); plt.title(f'{vcol}')
...
plt.xlabel(f'Nscore {vcol}'); plt.ylabel('Frequency'); plt.title(f'Nscore {vcol}')
plt.legend(loc='upper left')
plt.grid(True)
plt.subplots_adjust(left=0.0, bottom=0.0, right=2.0, top=2.2, wspace=0.2, hspace=0.3)
if save == True:
    plt.savefig(f'{wdsave}{inputfilename}_{vcol}Hist{extension_png}',dpi=300, bbox_inches = "tight")
if plot == True:
    plt.show()
else:
    plt.close()
...
# Cumulative
plt.subplot(121)                                        # plot original and normalised histograms
plt.hist(df[f'{vcol}'], facecolor='red',bins=np.linspace(vmin,vmax,1000), histtype="stepfilled",alpha=0.2,density=True,cumulative=True,edgecolor='black',label='Original')
# plt.xlim([0.05,0.25
# plt.ylim([0,5.0])
plt.xlabel(f'Cumulative {vcol}'); plt.ylabel('Frequency'); plt.title(f'Cumulative {vcol}')
plt.legend(loc='upper left')
plt.grid(True)
...
plt.xlim([-3.0,3.0]); plt.ylim([0,1.0])
plt.xlabel(f'Nscore Cumulative {vcol}'); plt.ylabel('Frequency'); plt.title(f'Nscore Cumulative {vcol}')
plt.legend(loc='upper left')
plt.grid(True)
plt.subplots_adjust(left=0.0, bottom=0.0, right=2.0, top=2.2, wspace=0.2, hspace=0.3)
if save == True:
    plt.savefig(f'{wdsave}{inputfilename}_Cum_{vcol}Hist{extension_png}',dpi=300, bbox_inches = "tight")
if plot == True:
    plt.show()
else:
    plt.close()
...
# plt.subplot(121)
# fracmap=plt.imshow(grid,cmap=cmap, interpolation='none')
# cbar = plt.colorbar(fracmap, orientation="vertical", ticks=np.linspace(amin(df['z']), amax(df['z']), 10))                                                                                        #contour map, colour bar and colourbar label???
# cbar.set_label('Z Value', rotation=270, labelpad=20)                                                                                                              ####################################################
# plt.title('NScore Fracture Map'); plt.xlabel('X Offset (px)'); plt.ylabel('Y Offset (px)')
# plt.subplots_adjust(left=0.0, bottom=0.0, right=2.0, top=0.8, wspace=0.5, hspace=0.3)
# plt.savefig(f'{wd}{inputfilename}_NScoreFractureMap{extension_png}',dpi=300, bbox_inches = "tight")
# plt.show()
# =============================================================================
...
cmap = plt.cm.plasma                    # color map
# Original
plt.scatter(df['x'], df['y'], c=df[f'{vcol}'], cmap=cmap, marker = ".", alpha=0.5) #alpha is transparency
...
plt.title(f'Original {vcol} Fracture map')
if save == True:
    plt.savefig(f'{wdsave}{inputfilename}_{vcol}MapOrig{extension_png}',dpi=300, bbox_inches = "tight")
plt.tight_layout()
if plot == True:
    plt.show()
else:
    plt.close()
#Normalised
plt.scatter(df['x'], df['y'], c=df[f'N{vcol}'], cmap=cmap, marker = ".", alpha=0.5) #alpha is transparency
plt.colorbar(mappable = None, label = f'Normalised {vcol} Value', orientation="vertical", ticks=np.linspace(amin(df[f'N{vcol}']), amax(df[f'N{vcol}']), 10))                                                                                      #contour map, colour bar and colourbar label???
plt.xlabel(r'$X (mm)$', fontsize=15),
plt.ylabel(r'$Y (mm)$', fontsize=15)
plt.title(f'Normalised {vcol} map')
if save == True:
    plt.savefig(f'{wdsave}{inputfilename}_{vcol}MapNorm{extension_png}',dpi=300, bbox_inches = "tight")
plt.tight_layout()
if plot == True:
    plt.show()
else:
...
# Second loop over the data: 
        for j in range(0,nd): 
# The lag:
            ydis = y[j] - y[i]                                                  #distance in y direction
            ...
            if iyl < 0 or iyl > nylag*2:                                        # check the pair has an acceptable distance in x-direction. accounting for 0,...,n-1 array indexing
                continue                                                        # Continue here means it disregards this pair and "continues"/moves onto the next
            xdis = x[j] - x[i]                                                  # distance in x-direction
...
#================================================================================================================================================================================ 
stop0 = time.time()
vmap, npmap = varmapv(df,'x','y', vcol,tmin=tmin,tmax=tmax,nxlag=nxlag,nylag=nylag,dxlag=dxlag,dylag=dylag,minnp=minnp,isill=isill)
...
plt.title(f'lag{dxlag} {vcol} Variogram Map'); plt.xlabel('X Offset (mm)'); plt.ylabel('Y Offset (mm)')

plt.subplots_adjust(left=0.0, bottom=0.0, right=1.0, top=1.2, wspace=0.2, hspace=0.2)
plt.tight_layout()
if save == True:
    plt.savefig(f'{wdsave}{inputfilename}_lag{dxlag}_{vcol}VariogramMap{extension_png}')
if plot == True:
    plt.show()
else:
    plt.close()
if save == True:
    exec(f'pd.DataFrame(vmap).to_csv("{wdsave}{inputfilename}_lag{dxlag}_{vcol}VariogramMap{extension_csv}")')
duration()
...
def gamv(
    ...
    for il in range(0, nlag + 2):
        if il%1000 ==0:                                  # Track the progress
            print('gamv Loop: ' + str(il)+"/" + str(nlag + 2) + ' ' + ctime())
...
for iazi in range(0,len(azi_mat)):                      # Loop over all directions
    ...
    plt.title(f'{inputfilename}{azi_mat[iazi]}deg_lag{dxlag} {vcol} Semi-Variogram')
    plt.xlim([0, lag_dist*(nlag+1) ])
    #plt.ylim([0, np.amax(gamf[gamf!=tmax])+0.25 ]) # Sometimes is useful if there are outliers
    plt.legend(loc='upper left')
    plt.grid(True)
    #plt.subplots_adjust(left=0.0, bottom=0.0, right=2.0, top=4.2, wspace=0.2, hspace=0.3)
    if save == True:
        plt.savefig(f'{wdsave}{inputfilename}{vcol}_Semi-variograms_lag{dxlag}_{azi_mat[iazi]}deg{extension_png}',dpi=300, bbox_inches = "tight")
    if plot == True:
        plt.show()
    else:
        plt.close()
